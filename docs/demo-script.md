...
9. Optional live lookup.
   ...
   - Show `Redacted review packet`, keep `Compact` selected, click `Copy compact markdown`, and point out that it creates a short public risk note with the hash reference, active thresholds, freshness verdict, redacted comparison headline, and capped top cues.
   - Switch to `Full`, click `Copy full markdown`, and point out that the detailed packet adds disclosed market rows, loaded public context rows, threshold detail, comparison detail, watchlist cues, and hash-reference-only caveats without exposing the full snapshot.
