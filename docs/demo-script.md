...
9. Optional live lookup.
   ...
   - Show `Review packet`, click `Copy markdown`, click `Download .md`, and point out that it packages the hash, live recheck, compact local recheck-history trend, market regime, per-market regime rows, watchlist, active thresholds, loaded volatility buffer, assistant read, driver comparison, and market context for quick review.
