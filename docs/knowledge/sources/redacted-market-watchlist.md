...
- Thin disclosed liquidation distance: `<= 500` bps.
- Tight disclosed liquidation distance: `<= 1000` bps.
- Material adverse 24-hour price move: `>= 2%` against the disclosed side.
- Material funding cost or funding delta: `>= 1` bps from the user's disclosed
  side perspective.
- High public range: `>= 8%`, or at least half of the disclosed liquidation
  distance.
