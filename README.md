...
- `src/lib/receipts/receipt-recheck-history.ts` stores compact browser-local history rows for repeated local receipt live rechecks.
...
- The receipt risk assistant is local and deterministic in this build; it answers only from receipt, hash, live recheck, market regime rows, recheck watchlist, loaded volatility-buffer rows, risk-driver comparison rows, market-context rows, funding, and sampled account-value context.
